        ...
        function velocity_data = calc_velocity(rca, global_para, wave, phantom, scan, lag, M, min_p_db)
            ...
            for i = 1:scan.ori_shape(1)
                for j = 1:scan.ori_shape(2)
                    d_v_mat_RC = reshape(velocity_doppler.RC(i, j, :), [wave.N_theta, 1]) * 2 * (-1);
                    % V_RC = lsqnonneg(theta_mat, d_v_mat_RC); % 最小二乘求解
                    V_RC = pinv(theta_mat) * d_v_mat_RC; % 伪逆法求解
                    % V_RC = inv(theta_mat'*theta_mat)*theta_mat'*d_v_mat_RC;
                    V_data.RC(i, j, :) = V_RC;
                end
            end
            % CR
            ...
            for i = 1:scan.ori_shape(1)
                for j = 1:scan.ori_shape(2)
                    d_v_mat_CR = reshape(velocity_doppler.CR(i, j, :), [wave.N_theta, 1]) * 2 * (-1);
                    % V_CR = lsqnonneg(theta_mat, d_v_mat_CR); % 最小二乘求解
                    V_CR = pinv(theta_mat) * d_v_mat_CR; % 伪逆法求解
                    % V_CR = inv(theta_mat'*theta_mat)*theta_mat'*d_v_mat_CR;
                    V_data.CR(i, j, :) = V_CR;
                end
            end
